...
const restore_1 = __nccwpck_require__(95);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const [matchedDir, destinationDir] = yield (0, restore_1.restoreCache)(path, allKeys, cacheDir);
            // State for the post save action to pick up
            core.saveState("L_CACHE_MATCH" /* match */, matchedDir);
            core.saveState("L_CACHE_DST" /* destination */, destinationDir);
            core.saveState("L_CACHE_DIR" /* cacheDir */, cacheDir);
        }
        catch (e) {
...
function restoreCache(destination, restoreKeys, cacheDir) {
    return __awaiter(this, void 0, void 0, function* () {
        const absDestination = (0, utils_1.absPath)(destination);
        try {
            yield fs.promises.access(absDestination);
        }
        catch (err) {
            core.debug('Destination directory does not exist, creating.');
            yield fs.promises.mkdir(absDestination, { recursive: true });
        }
        let contents;
        try {
            contents = yield fs.promises.readdir(cacheDir, { withFileTypes: true });
        }
        catch (err) {
            if (err.code !== 'ENOENT')
                throw err;
        ...
        if (matchedKey && matchedDir) {
            ...
            yield (0, utils_1.copy)(absMatchedDir, absDestination);
            const now = new Date();
            yield fs.promises.utimes(absMatchedDir, now, now);
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(686);
